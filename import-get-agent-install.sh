...
scalr-ctl farms list-servers --farmId $farmId | jq -r '.data[] | select(.scalrAgent.deployCommand) | "\(.publicIp[0]),\(.scalrAgent.deployCommand)"' > deploy.txt
